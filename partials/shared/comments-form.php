<?php
$is_enable_mailcollector = get_field( 'is_enable_mailcollector', 'options' ) ?? false;
$comment_title           = get_field( 'comment_title');
$comment_title_language  = $comment_title ? $comment_title : Translate::get('comments');

//$is_rating = get_field('is_rating', $post->ID) ?? false;
//$is_reviews = get_field('is_reviews', $post->ID) ?? false;


if ( $is_enable_mailcollector ):
	$title = get_field( 'mailcollector_title', 'options' ) ?: 'Contact Us';
	$brand = get_field( 'mailcollector_brand', 'options' );
	render_feedback_form( $title, $brand, true, $post->ID );
    app_get_comment_list($post);

else: ?>
    <footer class="cg-section" id="comments">
        <div class="container">
            <div class="cg-comments-sect">
                <h2><?= $comment_title_language ?></h2>
                    <form class="cg-form">
                        <input class="cg-form__item" name="name" type="text" aria-label="<?= Translate::get('placeholder_name') ?>" placeholder="<?= Translate::get('placeholder_name') ?>">
                        <input class="cg-form__item" name="email" type="email" aria-label="<?= Translate::get('placeholder_email') ?>" placeholder="<?= Translate::get('placeholder_email') ?>">
                        <textarea class="cg-form__item cg-form__item_textarea" name="comment" placeholder="<?= Translate::get('placeholder_comment') ?>" aria-label="<?= Translate::get('placeholder_comment') ?>"></textarea>
                        <button type="button" class="site-button site-button_gradient comment-form__button"><?= Translate::get('comment_button') ?></button>
                    <span class="comment-form__alert"></span>
                    </form>
                <?php app_get_comment_list( $post ); ?>
            </div>
        </div>
    </footer>
<?php endif;
