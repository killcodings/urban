...
function app_get_image( $params, $is_lazy = true, $alt = false ): string {
	...
	if ( ! $is_lazy ) {
        if ($alt) {
	        $image_str = wp_get_attachment_image( $params['id'], 'full', false, [
		        'alt' => $alt ? $alt : '',
		        'loading' => 'eager'
	        ] );
        } else {
...
function app_get_button( $button, $class = '', $relations = null, $custom_colors = null, $icon_left = false, $icon_right = false ): string {
	...
	if ( $custom_colors ) {
		...
		$border_radius    = $custom_colors['border_radius'] ?? '50';
...
<script type="application/ld+json">
{
	...
	"contentUrl": "{$args['youtube_video_url']}",
	...
	if ( $comments_list ): ?>
        <div class="cg-comments">
			<?php foreach ( $comments_list as $comment ):
				...
				$comment_ref_date     = get_comment_date( 'c', $comment_id );
...
function getInputHead() {
	global $post;
	$head_metric_group = get_field('head_metric_group', 'options') ?? false;
	$head_metric_input = $head_metric_group['head_metric_field'];
	$head_metric_relationship = $head_metric_group['head_metric_relationship'] ?? false;
	$current_id = $post->ID;
    if (!!$head_metric_relationship || (is_array($head_metric_relationship))) {
	    $is_current_id = in_array($current_id, $head_metric_relationship);
	    if ($is_current_id) {
		    return $head_metric_input;
	    }
    }
	return;
}
