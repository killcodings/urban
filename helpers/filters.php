...
function filter_bonuses( $content ) {
	$global_bonus = get_field( 'bonus', 'options' );
	foreach ( $global_bonus as $bonus ) {
		$content = str_replace( '[' . $bonus['key'] . ']', $bonus['value'], $content );
	}

	return $content;
}

add_filter( 'the_content', 'filter_bonuses' );


function modify_admin_comment_types_dropdown_defaults() {
	$comment_types = array(
		'comment' => __( 'Comments' ),
		'pings'   => __( 'Pings' ),
